...
There are hundreds more [[attribute variable\|attribute variable]]s that are considered important, but not all are readily amenable to experimental manipulations with random assignment. 
...
Several design considerations greatly enhance the usefulness of causal comparative research in education, including how groups are formed (selected), how control procedures (e.g., [[1. Nodes/matching\|matching]]) are used, how persuasively alternative explanations are ruled out (e.g., by [[1. Nodes/strong inference\|strong inference]]), and how well spurious relationships are uncovered and controlled.

## Design controls
For example, a [[1. Nodes/matched control group\|matched control group]] design would involve selecting two groups that are dissimilar on the hypothesized cause, say, bottle-fed and breastfed babies, but are the same on a matching variable believed to be a rival explanation, say, the age and socioeconomic status of the mother. (pg. 8)
...
**Method**:
1. **Participants**: Select groups of students from various schools who have been classified based on their parenting style.
2. **Data Collection**: Gather academic records (like GPA) and conduct interviews or surveys with students about their perceptions of their parents' styles.
3. **Analysis**: Compare the academic performance of students from each parenting style group to identify any significant differences.
